...
语言：[English](../../ECORE_REFERENCE.md) | [Español](../es/ECORE_REFERENCE.md) | **中文**

Model2Blockly 也可以直接从带注解的 Ecore 元模型生成 Blockly 编辑器。这条路线不使用
`.model2blockly` 关键字，而是使用标准 Ecore 结构，加上 `EAnnotation`。
...
| `EAttribute.lowerBound >= 1` | required validation |
| `EAttribute.iD=true` | ID 和唯一性 validation |
...
validation 可以来自 cardinality、ID、unique，也可以来自注解。
