// Code generators for domain "Expressionvalidation".
...
javascript.javascriptGenerator.forBlock['Range'] = function(block) {
  var label = block.getFieldValue('label');
  var min = block.getFieldValue('min');
  var max = block.getFieldValue('max');
  return '{' +   '"_type": "Range", "_blockId": ' + JSON.stringify(block.id) +  ', "label": ' + JSON.stringify(label) +  ', "min": ' + JSON.stringify(min) +  ', "max": ' + JSON.stringify(max) + ' },\n';
};
...
'Range': {
  ...
  fieldTypes: {'label': 'TEXT', 'min': 'INTEGER', 'max': 'INTEGER'},
  references: [],
  values: [],
  statements: []
}
