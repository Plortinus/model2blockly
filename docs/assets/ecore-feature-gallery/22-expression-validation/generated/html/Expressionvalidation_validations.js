// Validations for domain "Expressionvalidation".
...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Range') {
			var result = evaluateValidationExpression(block, "number(\"min\") <= number(\"max\")");
			if (!result.ok) {
				var message = "Minimum must be less than or equal to maximum.";
				if (result.error) message += ' Expression error: ' + result.error;
				queueWarn(block, message);
			}
		}
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Range') {
			var result = evaluateValidationExpression(block, "has(\"label\")");
			if (!result.ok) {
				var message = "Range label is required.";
				if (result.error) message += ' Expression error: ' + result.error;
				queueWarn(block, message);
			}
		}
