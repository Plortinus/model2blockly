// Code generators for domain "Autocategory".
...
javascript.javascriptGenerator.forBlock['Sprite'] = function(block) {
  var commands_code = javascript.javascriptGenerator.statementToCode(block, 'commands');
  var commands_stmt = (commands_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "Sprite", "_blockId": ' + JSON.stringify(block.id) +   ', "commands": ' + (commands_stmt ? '[' + commands_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['Move'] = function(block) {
  var steps = block.getFieldValue('steps');
  return '{' +   '"_type": "Move", "_blockId": ' + JSON.stringify(block.id) +  ', "steps": ' + JSON.stringify(steps) + ' },\n';
};
javascript.javascriptGenerator.forBlock['Turn'] = function(block) {
  var degrees = block.getFieldValue('degrees');
  return '{' +   '"_type": "Turn", "_blockId": ' + JSON.stringify(block.id) +  ', "degrees": ' + JSON.stringify(degrees) + ' },\n';
};
javascript.javascriptGenerator.forBlock['Say'] = function(block) {
  var text = block.getFieldValue('text');
  return '{' +   '"_type": "Say", "_blockId": ' + JSON.stringify(block.id) +  ', "text": ' + JSON.stringify(text) + ' },\n';
};
