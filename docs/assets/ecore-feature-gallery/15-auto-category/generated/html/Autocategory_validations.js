// Validations for domain "Autocategory".
...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Sprite') {
			var count = 0;
			var child = block.getInputTargetBlock('commands');
			while (child) { count++; child = child.getNextBlock(); }
			if (count < 1)
				queueWarn(block, '"Sprite.commands" needs at least 1 element(s). Has: ' + count);
			if (count > 4)
				queueWarn(block, '"Sprite.commands" allows at most 4 element(s). Has: ' + count);
		}
	});
	return warnings;
}
