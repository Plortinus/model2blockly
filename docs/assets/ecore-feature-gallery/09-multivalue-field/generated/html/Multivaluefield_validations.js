// Validations for domain "Multivaluefield".
...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'TagSet') {
			var val = block.getFieldValue('name');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"TagSet.name" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'TagSet') {
			...
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"TagSet.tags" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'TagSet') {
			...
			if (duplicates.length)
				queueWarn(block, '"TagSet.tags" values must be unique. Duplicate: ' + duplicates.join(', '));
		}
	});
	return warnings;
}
