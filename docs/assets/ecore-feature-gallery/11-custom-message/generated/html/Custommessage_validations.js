// Validations for domain "Custommessage".
...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Rule') {
			var val = block.getFieldValue('condition');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"Rule.condition" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Rule') {
			var val = block.getFieldValue('result');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"Rule.result" is required (cannot be empty).');
		}
	});
	return warnings;
}
