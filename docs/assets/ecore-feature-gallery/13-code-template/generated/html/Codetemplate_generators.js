// Code generators for domain "Codetemplate".
...
javascript.javascriptGenerator.forBlock['ApiCall'] = function(block) {
  var url = block.getFieldValue('url');
  var method = block.getFieldValue('method');
  return '{' +   '"_type": "ApiCall", "_blockId": ' + JSON.stringify(block.id) +  ', "url": ' + JSON.stringify(url) +  ', "method": ' + JSON.stringify(method) + ' },\n';
};
...
'ApiCall': {
  label: 'ApiCall',
  template: 'fetch(\"{{url}}\", { method: \"{{method}}\" });',
  fields: ['url', 'method'],
  fieldTypes: {'url': 'TEXT', 'method': 'DROPDOWN'},
  references: [],
  values: [],
  statements: []
}
