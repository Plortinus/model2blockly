// Validations for domain "Nestedcategory".
...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Screen') {
			var count = 0;
			var child = block.getInputTargetBlock('controls');
			while (child) { count++; child = child.getNextBlock(); }
			if (count < 1)
				queueWarn(block, '"Screen.controls" needs at least 1 element(s). Has: ' + count);
			if (count > 4)
				queueWarn(block, '"Screen.controls" allows at most 4 element(s). Has: ' + count);
		}
	});
	return warnings;
}
