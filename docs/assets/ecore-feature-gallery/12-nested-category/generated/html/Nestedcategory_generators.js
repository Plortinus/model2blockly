// Code generators for domain "Nestedcategory".
...
javascript.javascriptGenerator.forBlock['Screen'] = function(block) {
  var controls_code = javascript.javascriptGenerator.statementToCode(block, 'controls');
  var controls_stmt = (controls_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "Screen", "_blockId": ' + JSON.stringify(block.id) +   ', "controls": ' + (controls_stmt ? '[' + controls_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['TextInput'] = function(block) {
  var label = block.getFieldValue('label');
  return '{' +   '"_type": "TextInput", "_blockId": ' + JSON.stringify(block.id) +  ', "label": ' + JSON.stringify(label) + ' },\n';
};
javascript.javascriptGenerator.forBlock['SubmitButton'] = function(block) {
  var label = block.getFieldValue('label');
  return '{' +   '"_type": "SubmitButton", "_blockId": ' + JSON.stringify(block.id) +  ', "label": ' + JSON.stringify(label) + ' },\n';
};
