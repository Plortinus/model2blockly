// Code generators for domain "Fieldoverrides".
...
javascript.javascriptGenerator.forBlock['ControlSettings'] = function(block) {
  var caption = block.getFieldValue('caption');
  var ratio = block.getFieldValue('ratio');
  var locked = block.getFieldValue('locked');
  return '{' +   '"_type": "ControlSettings", "_blockId": ' + JSON.stringify(block.id) +  ', "caption": ' + JSON.stringify(caption) +  ', "ratio": ' + JSON.stringify(ratio) +  ', "locked": ' + JSON.stringify(locked) + ' },\n';
};
...
'ControlSettings': {
  ...
  template: null,
  fields: ['caption', 'ratio', 'locked'],
  fieldTypes: {'caption': 'TEXT', 'ratio': 'FLOAT', 'locked': 'BOOLEAN'},
  references: [],
  values: [],
  statements: []
}
