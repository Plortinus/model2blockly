// Code generators for domain "Visualfields".
...
javascript.javascriptGenerator.forBlock['IconBadge'] = function(block) {
  var icon = block.getFieldValue('icon');
  var rotation = block.getFieldValue('rotation');
  var caption = block.getFieldValue('caption');
  return '{' +   '"_type": "IconBadge", "_blockId": ' + JSON.stringify(block.id) +  ', "icon": ' + JSON.stringify(icon) +  ', "rotation": ' + JSON.stringify(rotation) +  ', "caption": ' + JSON.stringify(caption) + ' },\n';
};
...
'IconBadge': {
  label: 'IconBadge',
  template: null,
  fields: ['icon', 'rotation', 'caption'],
  fieldTypes: {'icon': 'IMAGE', 'rotation': 'ANGLE', 'caption': 'LABEL'},
  references: [],
  values: [],
  statements: []
}
