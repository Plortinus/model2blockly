// Validations for domain "Oclvalidation".
...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Checklist') {
			var result = evaluateValidationExpression(block, "has(\"name\")");
			if (!result.ok) {
				var message = "Constraint \"HasName\" is violated on Checklist.";
				if (result.error) message += ' Expression error: ' + result.error;
				queueWarn(block, message);
			}
		}
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Checklist') {
			var result = evaluateValidationExpression(block, "size(\"items\") >= 1");
			if (!result.ok) {
				var message = "Constraint \"HasItems\" is violated on Checklist.";
				if (result.error) message += ' Expression error: ' + result.error;
				queueWarn(block, message);
			}
		}
