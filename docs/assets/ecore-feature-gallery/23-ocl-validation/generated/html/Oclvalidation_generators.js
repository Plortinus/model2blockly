// Code generators for domain "Oclvalidation".
...
javascript.javascriptGenerator.forBlock['Checklist'] = function(block) {
  var name = block.getFieldValue('name');
  var items_code = javascript.javascriptGenerator.statementToCode(block, 'items');
  var items_stmt = (items_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "Checklist", "_blockId": ' + JSON.stringify(block.id) +  ', "name": ' + JSON.stringify(name) +   ', "items": ' + (items_stmt ? '[' + items_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['ChecklistItem'] = function(block) {
  var text = block.getFieldValue('text');
  return '{' +   '"_type": "ChecklistItem", "_blockId": ' + JSON.stringify(block.id) +  ', "text": ' + JSON.stringify(text) + ' },\n';
};
