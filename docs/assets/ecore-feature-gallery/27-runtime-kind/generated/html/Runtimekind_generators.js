// Code generators for domain "Runtimekind".
...
javascript.javascriptGenerator.forBlock['App'] = function(block) {
  ...
  return '{' +   '"_type": "App", "_blockId": ' + JSON.stringify(block.id) +  ', "name": ' + JSON.stringify(name) +   ', "pages": ' + (pages_stmt ? '[' + pages_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['Page'] = function(block) {
  var title = block.getFieldValue('title');
  return '{' +   '"_type": "Page", "_blockId": ' + JSON.stringify(block.id) +  ', "title": ' + JSON.stringify(title) + ' },\n';
};
