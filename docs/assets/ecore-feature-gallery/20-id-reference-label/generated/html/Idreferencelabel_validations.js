// Validations for domain "Idreferencelabel".
...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Library') {
			var count = 0;
			var child = block.getInputTargetBlock('authors');
			while (child) { count++; child = child.getNextBlock(); }
			if (count < 1)
				queueWarn(block, '"Library.authors" needs at least 1 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Library') {
			var count = 0;
			var child = block.getInputTargetBlock('books');
			while (child) { count++; child = child.getNextBlock(); }
			if (count < 1)
				queueWarn(block, '"Library.books" needs at least 1 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Author') {
			var val = block.getFieldValue('authorId');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"Author.authorId" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Book') {
			var val = block.getFieldValue('author');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"Book.author" is required (cannot be empty).');
		}
	});
	(function() {
		var seen = {};
		workspace.getAllBlocks(false).forEach(function(block) {
			if (block.type === 'Author') {
				...
				if (!val) return;
				if (seen[val]) {
					queueWarn(block, '"Author.authorId" must be unique. Duplicate ID: ' + val);
					queueWarn(seen[val], '"Author.authorId" must be unique. Duplicate ID: ' + val);
				} else {
					seen[val] = block;
				}
			}
