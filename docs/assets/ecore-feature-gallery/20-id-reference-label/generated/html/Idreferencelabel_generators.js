// Code generators for domain "Idreferencelabel".
...
javascript.javascriptGenerator.forBlock['Library'] = function(block) {
  var authors_code = javascript.javascriptGenerator.statementToCode(block, 'authors');
  var authors_stmt = (authors_code || '').trim().replace(/,\\s*$/, '');
  var books_code = javascript.javascriptGenerator.statementToCode(block, 'books');
  var books_stmt = (books_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "Library", "_blockId": ' + JSON.stringify(block.id) +   ', "authors": ' + (authors_stmt ? '[' + authors_stmt + ']' : '[]') +   ', "books": ' + (books_stmt ? '[' + books_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['Author'] = function(block) {
  var authorId = block.getFieldValue('authorId');
  var displayName = block.getFieldValue('displayName');
  return '{' +   '"_type": "Author", "_blockId": ' + JSON.stringify(block.id) +  ', "_modelId": ' + JSON.stringify(authorId || block.id) +  ', "authorId": ' + JSON.stringify(authorId) +  ', "displayName": ' + JSON.stringify(displayName) + ' },\n';
};
javascript.javascriptGenerator.forBlock['Book'] = function(block) {
  var title = block.getFieldValue('title');
  var author = block.getFieldValue('author');
  return '{' +   '"_type": "Book", "_blockId": ' + JSON.stringify(block.id) +  ', "title": ' + JSON.stringify(title) +  ', "author": ' + JSON.stringify(author) + ' },\n';
};
...
'Library': {
  ...
  references: [],
  values: [],
  statements: ['authors', 'books']
},
'Author': {
  label: 'Author',
  template: null,
  fields: ['authorId', 'displayName'],
  fieldTypes: {'authorId': 'TEXT', 'displayName': 'TEXT'},
  references: [],
  values: [],
  statements: []
},
