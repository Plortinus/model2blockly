// Validations for domain "Abstracttype".
...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Screen') {
			...
			if (count < 1)
				queueWarn(block, '"Screen.components" needs at least 1 element(s). Has: ' + count);
			if (count > 4)
				queueWarn(block, '"Screen.components" allows at most 4 element(s). Has: ' + count);
		}
	});
	return warnings;
}
