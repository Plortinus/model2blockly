// Code generators for domain "Classpresentation".
...
javascript.javascriptGenerator.forBlock['Panel'] = function(block) {
  var title = block.getFieldValue('title');
  var width = block.getFieldValue('width');
  return '{' +   '"_type": "Panel", "_blockId": ' + JSON.stringify(block.id) +  ', "title": ' + JSON.stringify(title) +  ', "width": ' + JSON.stringify(width) + ' },\n';
};
...
'Panel': {
  ...
  template: null,
  fields: ['title', 'width'],
  fieldTypes: {'title': 'TEXT', 'width': 'INTEGER'},
  references: [],
  values: [],
  statements: []
}
