// Validations for domain "Uimetadata".
...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'ProfileForm') {
			var val = block.getFieldValue('displayName');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"ProfileForm.displayName" is required (cannot be empty).');
		}
	});
	return warnings;
}
