// Code generators for domain "Uimetadata".
...
javascript.javascriptGenerator.forBlock['ProfileForm'] = function(block) {
  var displayName = block.getFieldValue('displayName');
  var slug = block.getFieldValue('slug');
  var internalToken = block.getFieldValue('internalToken');
  return '{' +   '"_type": "ProfileForm", "_blockId": ' + JSON.stringify(block.id) +  ', "displayName": ' + JSON.stringify(displayName) +  ', "slug": ' + JSON.stringify(slug) +  ', "internalToken": ' + JSON.stringify(internalToken) + ' },\n';
};
...
'ProfileForm': {
  label: 'Profile form',
  template: null,
  fields: ['displayName', 'slug', 'internalToken'],
  fieldTypes: {'displayName': 'TEXT', 'slug': 'TEXT', 'internalToken': 'TEXT'},
  references: [],
  values: [],
  statements: []
}
