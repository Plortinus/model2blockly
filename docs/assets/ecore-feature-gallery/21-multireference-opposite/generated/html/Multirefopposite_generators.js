// Code generators for domain "Multirefopposite".
...
javascript.javascriptGenerator.forBlock['Organization'] = function(block) {
  var teams_code = javascript.javascriptGenerator.statementToCode(block, 'teams');
  var teams_stmt = (teams_code || '').trim().replace(/,\\s*$/, '');
  var members_code = javascript.javascriptGenerator.statementToCode(block, 'members');
  var members_stmt = (members_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "Organization", "_blockId": ' + JSON.stringify(block.id) +   ', "teams": ' + (teams_stmt ? '[' + teams_stmt + ']' : '[]') +   ', "members": ' + (members_stmt ? '[' + members_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['Team'] = function(block) {
  var name = block.getFieldValue('name');
  var members = parseBlocklyListField(block.getFieldValue('members'));
  return '{' +   '"_type": "Team", "_blockId": ' + JSON.stringify(block.id) +  ', "_modelId": ' + JSON.stringify(name || block.id) +  ', "name": ' + JSON.stringify(name) +  ', "members": ' + JSON.stringify(members) + ' },\n';
};
javascript.javascriptGenerator.forBlock['Member'] = function(block) {
  var name = block.getFieldValue('name');
  var team = block.getFieldValue('team');
  return '{' +   '"_type": "Member", "_blockId": ' + JSON.stringify(block.id) +  ', "_modelId": ' + JSON.stringify(name || block.id) +  ', "name": ' + JSON.stringify(name) +  ', "team": ' + JSON.stringify(team) + ' },\n';
};
