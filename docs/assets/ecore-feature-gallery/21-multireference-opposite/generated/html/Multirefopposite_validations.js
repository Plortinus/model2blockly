// Validations for domain "Multirefopposite".
...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Organization') {
			var count = 0;
			var child = block.getInputTargetBlock('teams');
			while (child) { count++; child = child.getNextBlock(); }
			if (count < 1)
				queueWarn(block, '"Organization.teams" needs at least 1 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Organization') {
			var count = 0;
			var child = block.getInputTargetBlock('members');
			while (child) { count++; child = child.getNextBlock(); }
			if (count < 1)
				queueWarn(block, '"Organization.members" needs at least 1 element(s). Has: ' + count);
		}
	});
	(function() {
		var seen = {};
		workspace.getAllBlocks(false).forEach(function(block) {
			if (block.type === 'Team') {
				...
				if (!val) return;
				if (seen[val]) {
					queueWarn(block, '"Team.name" must be unique. Duplicate ID: ' + val);
					queueWarn(seen[val], '"Team.name" must be unique. Duplicate ID: ' + val);
				} else {
					seen[val] = block;
				}
			}
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Team') {
			var duplicates = duplicateValues(fieldValues(block.getFieldValue('members')));
			if (duplicates.length)
				queueWarn(block, '"Team.members" values must be unique. Duplicate: ' + duplicates.join(', '));
		}
	});
	(function() {
		var seen = {};
		workspace.getAllBlocks(false).forEach(function(block) {
			if (block.type === 'Member') {
				...
				if (!val) return;
				if (seen[val]) {
					queueWarn(block, '"Member.name" must be unique. Duplicate ID: ' + val);
					queueWarn(seen[val], '"Member.name" must be unique. Duplicate ID: ' + val);
				} else {
					seen[val] = block;
				}
			}
