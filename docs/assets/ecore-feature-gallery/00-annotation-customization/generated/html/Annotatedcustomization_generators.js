// Code generators for domain "Annotatedcustomization".
...
javascript.javascriptGenerator.forBlock['TaskBoard'] = function(block) {
  var name = block.getFieldValue('name');
  var tasks_code = javascript.javascriptGenerator.statementToCode(block, 'tasks');
  var tasks_stmt = (tasks_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "TaskBoard", "_blockId": ' + JSON.stringify(block.id) +  ', "name": ' + JSON.stringify(name) +   ', "tasks": ' + (tasks_stmt ? '[' + tasks_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['Task'] = function(block) {
  ...
  return '{' +   '"_type": "Task", "_blockId": ' + JSON.stringify(block.id) +  ', "title": ' + JSON.stringify(title) +  ', "done": ' + JSON.stringify(done) +  ', "priority": ' + JSON.stringify(priority) + ' },\n';
};
...
'Task': {
  label: 'Task',
  template: null,
  fields: ['title', 'done', 'priority'],
  fieldTypes: {'title': 'TEXT', 'done': 'BOOLEAN', 'priority': 'DROPDOWN'},
  references: [],
  values: [],
  statements: []
}
