// Validations for domain "Annotatedcustomization".
...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'TaskBoard') {
			var count = 0;
			var child = block.getInputTargetBlock('tasks');
			while (child) { count++; child = child.getNextBlock(); }
			if (count < 1)
				queueWarn(block, '"TaskBoard.tasks" needs at least 1 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Task') {
			...
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"Task.title" is required (cannot be empty).');
		}
	});
	return warnings;
}
