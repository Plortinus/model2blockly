// Code generators for domain "Statementinput".
...
javascript.javascriptGenerator.forBlock['Page'] = function(block) {
  ...
  return '{' +   '"_type": "Page", "_blockId": ' + JSON.stringify(block.id) +  ', "title": ' + JSON.stringify(title) +   ', "components": ' + (components_stmt ? '[' + components_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['Button'] = function(block) {
  var label = block.getFieldValue('label');
  return '{' +   '"_type": "Button", "_blockId": ' + JSON.stringify(block.id) +  ', "label": ' + JSON.stringify(label) + ' },\n';
};
