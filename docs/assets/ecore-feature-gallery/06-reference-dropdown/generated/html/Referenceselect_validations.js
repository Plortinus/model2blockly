// Validations for domain "Referenceselect".
...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Flow') {
			...
			if (count < 1)
				queueWarn(block, '"Flow.pages" needs at least 1 element(s). Has: ' + count);
			if (count > 3)
				queueWarn(block, '"Flow.pages" allows at most 3 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Flow') {
			...
			while (child) { count++; child = child.getNextBlock(); }
			if (count < 1)
				queueWarn(block, '"Flow.actions" needs at least 1 element(s). Has: ' + count);
			if (count > 3)
				queueWarn(block, '"Flow.actions" allows at most 3 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
	...
	(function() {
		...
		workspace.getAllBlocks(false).forEach(function(block) {
			if (block.type === 'Page') {
				...
				if (!val) return;
				if (seen[val]) {
					queueWarn(block, '"Page.name" must be unique. Duplicate ID: ' + val);
					queueWarn(seen[val], '"Page.name" must be unique. Duplicate ID: ' + val);
				} else {
					seen[val] = block;
				}
			}
