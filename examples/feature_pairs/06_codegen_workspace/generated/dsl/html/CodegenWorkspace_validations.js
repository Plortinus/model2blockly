// Validations for domain "CodegenWorkspace".
...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Program') {
			var count = 0;
			var child = block.getInputTargetBlock('steps');
			while (child) { count++; child = child.getNextBlock(); }
			if (count < 1)
				queueWarn(block, '"Program.steps" needs at least 1 element(s). Has: ' + count);
			if (count > 4)
				queueWarn(block, '"Program.steps" allows at most 4 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Program') {
			...
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"Program.name" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Log') {
			...
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"Log.message" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Wait') {
			var val = block.getFieldValue('milliseconds');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"Wait.milliseconds" is required (cannot be empty).');
		}
	});
	return warnings;
}
