// Code generators for domain "CodegenWorkspace".
...
javascript.javascriptGenerator.forBlock['Program'] = function(block) {
  var name = block.getFieldValue('name');
  var steps_code = javascript.javascriptGenerator.statementToCode(block, 'steps');
  var steps_stmt = (steps_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "Program", "_blockId": ' + JSON.stringify(block.id) +  ', "name": ' + JSON.stringify(name) +   ', "steps": ' + (steps_stmt ? '[' + steps_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['Log'] = function(block) {
  var message = block.getFieldValue('message');
  return '{' +   '"_type": "Log", "_blockId": ' + JSON.stringify(block.id) +  ', "message": ' + JSON.stringify(message) + ' },\n';
};
javascript.javascriptGenerator.forBlock['Wait'] = function(block) {
  var milliseconds = block.getFieldValue('milliseconds');
  return '{' +   '"_type": "Wait", "_blockId": ' + JSON.stringify(block.id) +  ', "milliseconds": ' + JSON.stringify(milliseconds) + ' },\n';
};
javascript.javascriptGenerator.forBlock['BooleanLiteral'] = function(block) {
  var literal = block.getFieldValue('literal');
  var code = '{' +   '"_type": "BooleanLiteral", "_blockId": ' + JSON.stringify(block.id) +  ', "literal": ' + JSON.stringify(literal) + ' }';
  return [code, 0];
};
...
'Program': {
  label: 'Program',
  template: 'program(\'{{name}}\', () => { {{statements:steps}} });',
  fields: ['name'],
...
'Wait': {
  label: 'Wait',
  template: 'wait({{milliseconds}});',
  fields: ['milliseconds'],
  fieldTypes: {'milliseconds': 'INTEGER'},
  references: [],
  values: [],
  statements: []
},
