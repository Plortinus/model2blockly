// Validations for domain "ValidatedApp".
...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'App') {
			var result = evaluateValidationExpression(block, "! bool(\"published\") || has(\"tags\")");
			if (!result.ok) {
				var message = "A published app must have at least one tag.";
				if (result.error) message += ' Expression error: ' + result.error;
				queueWarn(block, message);
			}
		}
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Page') {
			var result = evaluateValidationExpression(block, "size(\"components\") >= 1 || value(\"description\") !== \'\'");
			if (!result.ok) {
				var message = "A page must have at least one component or a non-empty description.";
				if (result.error) message += ' Expression error: ' + result.error;
				queueWarn(block, message);
			}
		}
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Button') {
			var result = evaluateValidationExpression(block, "! bool(\"enabled\") || number(\"width\") >= 100");
			if (!result.ok) {
				var message = "An enabled button must be at least 100 pixels wide.";
				if (result.error) message += ' Expression error: ' + result.error;
				queueWarn(block, message);
			}
		}
