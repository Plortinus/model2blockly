// Validations for domain "FieldsUi".
...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'ContentForm') {
			var val = block.getFieldValue('title');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"ContentForm.title" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'ContentForm') {
			var val = block.getFieldValue('tags');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"ContentForm.tags" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'ContentForm') {
			...
			if (count > 3)
				queueWarn(block, '"ContentForm.tags" allows at most 3 value(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'ContentForm') {
			...
			if (duplicates.length)
				queueWarn(block, '"ContentForm.tags" values must be unique. Duplicate: ' + duplicates.join(', '));
		}
	});
	return warnings;
}
