// Validations for domain "CompositionInputs".
...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Document') {
			var count = 0;
			var child = block.getInputTargetBlock('sections');
			while (child) { count++; child = child.getNextBlock(); }
			if (count < 1)
				queueWarn(block, '"Document.sections" needs at least 1 element(s). Has: ' + count);
			if (count > 4)
				queueWarn(block, '"Document.sections" allows at most 4 element(s). Has: ' + count);
		}
	});
	return warnings;
}
