// Code generators for domain "CompositionInputs".
...
javascript.javascriptGenerator.forBlock['Document'] = function(block) {
  var title = block.getFieldValue('title');
  var sections_code = javascript.javascriptGenerator.statementToCode(block, 'sections');
  var sections_stmt = (sections_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "Document", "_blockId": ' + JSON.stringify(block.id) +  ', "title": ' + JSON.stringify(title) +   ', "sections": ' + (sections_stmt ? '[' + sections_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['Paragraph'] = function(block) {
  var heading = block.getFieldValue('heading');
  var emphasis = block.getFieldValue('emphasis');
  var content_code = javascript.javascriptGenerator.valueToCode(block, 'content', 0) || 'null';
  return '{' +   '"_type": "Paragraph", "_blockId": ' + JSON.stringify(block.id) +  ', "heading": ' + JSON.stringify(heading) +  ', "emphasis": ' + JSON.stringify(emphasis) +  ', "content": ' + content_code + ' },\n';
};
...
'Paragraph': {
  label: 'Paragraph',
  template: null,
  fields: ['heading', 'emphasis'],
  fieldTypes: {'heading': 'TEXT', 'emphasis': 'BOOLEAN'},
  references: [],
  values: ['content'],
  statements: []
},
