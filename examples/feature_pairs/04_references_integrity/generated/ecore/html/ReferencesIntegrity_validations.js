// Validations for domain "ReferencesIntegrity".
...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Workflow') {
			var count = 0;
			var child = block.getInputTargetBlock('elements');
			while (child) { count++; child = child.getNextBlock(); }
			if (count < 1)
				queueWarn(block, '"Workflow.elements" needs at least 1 element(s). Has: ' + count);
			if (count > 6)
				queueWarn(block, '"Workflow.elements" allows at most 6 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Card') {
			var val = block.getFieldValue('cardId');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"Card.cardId" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Link') {
			var val = block.getFieldValue('linkId');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"Link.linkId" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Link') {
			...
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"Link.source" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Link') {
			...
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"Link.target" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Card') {
			var count = fieldMultiplicityCount(block.getFieldValue('outgoing'));
			if (count > 3)
				queueWarn(block, '"Card.outgoing" allows at most 3 value(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Link') {
			var count = fieldMultiplicityCount(block.getFieldValue('target'));
			if (count > 2)
				queueWarn(block, '"Link.target" allows at most 2 value(s). Has: ' + count);
		}
	});
	(function() {
		var seen = {};
		workspace.getAllBlocks(false).forEach(function(block) {
			if (block.type === 'Card') {
				...
				if (!val) return;
				if (seen[val]) {
					queueWarn(block, '"Card.cardId" must be unique. Duplicate ID: ' + val);
					queueWarn(seen[val], '"Card.cardId" must be unique. Duplicate ID: ' + val);
				} else {
					seen[val] = block;
				}
			}
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Card') {
			var duplicates = duplicateValues(fieldValues(block.getFieldValue('outgoing')));
			if (duplicates.length)
				queueWarn(block, '"Card.outgoing" values must be unique. Duplicate: ' + duplicates.join(', '));
		}
	});
	(function() {
		var seen = {};
		workspace.getAllBlocks(false).forEach(function(block) {
			if (block.type === 'Link') {
				...
				if (!val) return;
				if (seen[val]) {
					queueWarn(block, '"Link.linkId" must be unique. Duplicate ID: ' + val);
					queueWarn(seen[val], '"Link.linkId" must be unique. Duplicate ID: ' + val);
				} else {
					seen[val] = block;
				}
			}
