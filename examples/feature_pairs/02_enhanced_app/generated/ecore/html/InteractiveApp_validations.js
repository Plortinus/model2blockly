// Validations for domain "InteractiveApp".
...
function computeValidationWarnings(workspace) {
	...
	function computeTypedMultiValueWarnings(workspace, queueWarn) {
		workspace.getAllBlocks(false).forEach(function(block) {
			(block.inputList || []).forEach(function(input) {
				(input.fieldRow || []).forEach(function(field) {
					if (!field || typeof field.getValidationErrors !== 'function') return;
					field.getValidationErrors().forEach(function(message) {
						queueWarn(block, '"' + block.type + '.' + (field.name || 'values') + '": ' + message);
					});
				});
			});
		});
	}
	computeTypedMultiValueWarnings(workspace, queueWarn);
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'App') {
			...
			if (count > 5)
				queueWarn(block, '"App.pages" allows at most 5 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Page') {
			...
			while (child) { count++; child = child.getNextBlock(); }
			if (count > 10)
				queueWarn(block, '"Page.components" allows at most 10 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Container') {
			...
			while (child) { count++; child = child.getNextBlock(); }
			if (count < 1)
				queueWarn(block, '"Container.children" needs at least 1 element(s). Has: ' + count);
			if (count > 10)
				queueWarn(block, '"Container.children" allows at most 10 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'App') {
			var val = block.getFieldValue('previewWidths');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"App.previewWidths" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'App') {
			var val = block.getFieldValue('targetPlatforms');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"App.targetPlatforms" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Page') {
			var val = block.getFieldValue('id');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"Page.id" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Page') {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Label') {
			...
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"Label.text" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Button') {
			var val = block.getFieldValue('text');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"Button.text" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Image') {
			...
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"Image.url" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'NavigateAction') {
			var val = block.getFieldValue('target');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"NavigateAction.target" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'ShowMessageAction') {
			var val = block.getFieldValue('message');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"ShowMessageAction.message" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'App') {
			...
			if (count > 3)
				queueWarn(block, '"App.tags" allows at most 3 value(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'App') {
			var count = fieldMultiplicityCount(block.getFieldValue('previewWidths'));
			if (count > 4)
				queueWarn(block, '"App.previewWidths" allows at most 4 value(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'App') {
			var count = fieldMultiplicityCount(block.getFieldValue('targetPlatforms'));
			if (count > 3)
				queueWarn(block, '"App.targetPlatforms" allows at most 3 value(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'App') {
			var duplicates = duplicateValues(fieldValues(block.getFieldValue('tags')));
			if (duplicates.length)
				queueWarn(block, '"App.tags" values must be unique. Duplicate: ' + duplicates.join(', '));
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'App') {
			var duplicates = duplicateValues(fieldValues(block.getFieldValue('previewWidths')));
			if (duplicates.length)
				queueWarn(block, '"App.previewWidths" values must be unique. Duplicate: ' + duplicates.join(', '));
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'App') {
			var duplicates = duplicateValues(fieldValues(block.getFieldValue('targetPlatforms')));
			if (duplicates.length)
				queueWarn(block, '"App.targetPlatforms" values must be unique. Duplicate: ' + duplicates.join(', '));
		}
	});
	(function() {
		var seen = {};
		workspace.getAllBlocks(false).forEach(function(block) {
			if (block.type === 'Page') {
				...
				if (!val) return;
				if (seen[val]) {
					queueWarn(block, '"Page.id" must be unique. Duplicate ID: ' + val);
					queueWarn(seen[val], '"Page.id" must be unique. Duplicate ID: ' + val);
				} else {
					seen[val] = block;
				}
			}
