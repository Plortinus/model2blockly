...
javascript.javascriptGenerator.forBlock['Project'] = function(block) {
  var name = block.getFieldValue('name');
  var workItems_code = javascript.javascriptGenerator.statementToCode(block, 'workItems');
  var workItems_stmt = (workItems_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "Project", "_blockId": ' + JSON.stringify(block.id) +  ', "name": ' + JSON.stringify(name) +   ', "workItems": ' + (workItems_stmt ? '[' + workItems_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['Task'] = function(block) {
  var title = block.getFieldValue('title');
  return '{' +   '"_type": "Task", "_blockId": ' + JSON.stringify(block.id) +  ', "title": ' + JSON.stringify(title) + ' },\n';
};
javascript.javascriptGenerator.forBlock['Milestone'] = function(block) {
  var name = block.getFieldValue('name');
  return '{' +   '"_type": "Milestone", "_blockId": ' + JSON.stringify(block.id) +  ', "name": ' + JSON.stringify(name) + ' },\n';
};
javascript.javascriptGenerator.forBlock['TeamMember'] = function(block) {
  var name = block.getFieldValue('name');
  return '{' +   '"_type": "TeamMember", "_blockId": ' + JSON.stringify(block.id) +  ', "name": ' + JSON.stringify(name) + ' },\n';
};
