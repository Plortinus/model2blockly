// Validations for domain "BasicStructure".
...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Project') {
			var count = 0;
			var child = block.getInputTargetBlock('workItems');
			while (child) { count++; child = child.getNextBlock(); }
			if (count < 1)
				queueWarn(block, '"Project.workItems" needs at least 1 element(s). Has: ' + count);
			if (count > 6)
				queueWarn(block, '"Project.workItems" allows at most 6 element(s). Has: ' + count);
		}
	});
	return warnings;
}
