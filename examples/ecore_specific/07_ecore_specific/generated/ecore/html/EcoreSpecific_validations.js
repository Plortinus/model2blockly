// Validations for domain "EcoreSpecific".
...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Catalog') {
			var count = 0;
			var child = block.getInputTargetBlock('entries');
			while (child) { count++; child = child.getNextBlock(); }
			if (count < 1)
				queueWarn(block, '"Catalog.entries" needs at least 1 element(s). Has: ' + count);
			if (count > 4)
				queueWarn(block, '"Catalog.entries" allows at most 4 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Entry') {
			...
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"Entry.title" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Entry') {
			var val = block.getFieldValue('entryId');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"Entry.entryId" is required (cannot be empty).');
		}
	});
	(function() {
		var seen = {};
		workspace.getAllBlocks(false).forEach(function(block) {
			if (block.type === 'Entry') {
				...
				if (!val) return;
				if (seen[val]) {
					queueWarn(block, '"Entry.entryId" must be unique. Duplicate ID: ' + val);
					queueWarn(seen[val], '"Entry.entryId" must be unique. Duplicate ID: ' + val);
				} else {
					seen[val] = block;
				}
			}
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Entry') {
			...
			if (!result.ok) {
				var message = "Constraint \"HasTitle\" is violated on Entry.";
				if (result.error) message += ' Expression error: ' + result.error;
				queueWarn(block, message);
			}
		}
