// Code generators for domain "EcoreSpecific".
...
javascript.javascriptGenerator.forBlock['Catalog'] = function(block) {
  var name = block.getFieldValue('name');
  var featured = block.getFieldValue('featured');
  var entries_code = javascript.javascriptGenerator.statementToCode(block, 'entries');
  var entries_stmt = (entries_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "Catalog", "_blockId": ' + JSON.stringify(block.id) +  ', "name": ' + JSON.stringify(name) +  ', "featured": ' + JSON.stringify(featured) +   ', "entries": ' + (entries_stmt ? '[' + entries_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['Entry'] = function(block) {
  var title = block.getFieldValue('title');
  var entryId = block.getFieldValue('entryId');
  var longCount = block.getFieldValue('longCount');
  var shortCount = block.getFieldValue('shortCount');
  var bigCount = block.getFieldValue('bigCount');
  var doubleRatio = block.getFieldValue('doubleRatio');
  var decimalRatio = block.getFieldValue('decimalRatio');
  return '{' +   '"_type": "Entry", "_blockId": ' + JSON.stringify(block.id) +  ', "_modelId": ' + JSON.stringify(entryId || block.id) +  ', "title": ' + JSON.stringify(title) +  ', "entryId": ' + JSON.stringify(entryId) +  ', "longCount": ' + JSON.stringify(longCount) +  ', "shortCount": ' + JSON.stringify(shortCount) +  ', "bigCount": ' + JSON.stringify(bigCount) +  ', "doubleRatio": ' + JSON.stringify(doubleRatio) +  ', "decimalRatio": ' + JSON.stringify(decimalRatio) + ' },\n';
};
...
'Entry': {
  label: 'Entry',
  template: null,
  fields: ['title', 'entryId', 'longCount', 'shortCount', 'bigCount', 'doubleRatio', 'decimalRatio'],
  fieldTypes: {'title': 'TEXT', 'entryId': 'TEXT', 'longCount': 'INTEGER', 'shortCount': 'INTEGER', 'bigCount': 'INTEGER', 'doubleRatio': 'FLOAT', 'decimalRatio': 'FLOAT'},
  references: [],
  values: [],
  statements: []
}
