// Code generators for domain "Puzzle".
...
javascript.javascriptGenerator.forBlock['animal'] = function(block) {
  var NAME = block.getFieldValue('NAME');
  var LEGS = block.getFieldValue('LEGS');
  var PIC_code = javascript.javascriptGenerator.valueToCode(block, 'PIC', 0) || 'null';
  var TRAITS_code = javascript.javascriptGenerator.statementToCode(block, 'TRAITS');
  var TRAITS_stmt = (TRAITS_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "animal", "_blockId": ' + JSON.stringify(block.id) +  ', "NAME": ' + JSON.stringify(NAME) +  ', "LEGS": ' + JSON.stringify(LEGS) +  ', "PIC": ' + PIC_code +   ', "TRAITS": ' + (TRAITS_stmt ? '[' + TRAITS_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['picture'] = function(block) {
  var PIC_IMAGE = block.getFieldValue('PIC_IMAGE');
  var code = '{' +   '"_type": "picture", "_blockId": ' + JSON.stringify(block.id) +  ', "PIC_IMAGE": ' + JSON.stringify(PIC_IMAGE) + ' }';
  return [code, 0];
};
javascript.javascriptGenerator.forBlock['trait'] = function(block) {
  var NAME = block.getFieldValue('NAME');
  return '{' +   '"_type": "trait", "_blockId": ' + JSON.stringify(block.id) +  ', "NAME": ' + JSON.stringify(NAME) + ' },\n';
};
...
'animal': {
  ...
  template: null,
  fields: ['NAME', 'LEGS'],
  fieldTypes: {'NAME': 'LABEL', 'LEGS': 'DROPDOWN'},
  references: [],
  values: ['PIC'],
  statements: ['TRAITS']
},
