// Code generators for domain "Maze".
...
javascript.javascriptGenerator.forBlock['maze_moveForward'] = function(block) {
  return '{' +   '"_type": "maze_moveForward", "_blockId": ' + JSON.stringify(block.id) + ' },\n';
};
javascript.javascriptGenerator.forBlock['maze_turn'] = function(block) {
  var DIR = block.getFieldValue('DIR');
  return '{' +   '"_type": "maze_turn", "_blockId": ' + JSON.stringify(block.id) +  ', "DIR": ' + JSON.stringify(DIR) + ' },\n';
};
javascript.javascriptGenerator.forBlock['maze_forever'] = function(block) {
  ...
  return '{' +   '"_type": "maze_forever", "_blockId": ' + JSON.stringify(block.id) +  ', "MARKER": ' + JSON.stringify(MARKER) +   ', "DO": ' + (DO_stmt ? '[' + DO_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['maze_if'] = function(block) {
  var DIR = block.getFieldValue('DIR');
  var DO_code = javascript.javascriptGenerator.statementToCode(block, 'DO');
  var DO_stmt = (DO_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "maze_if", "_blockId": ' + JSON.stringify(block.id) +  ', "DIR": ' + JSON.stringify(DIR) +   ', "DO": ' + (DO_stmt ? '[' + DO_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['maze_ifElse'] = function(block) {
  var DIR = block.getFieldValue('DIR');
  var DO_code = javascript.javascriptGenerator.statementToCode(block, 'DO');
  var DO_stmt = (DO_code || '').trim().replace(/,\\s*$/, '');
  var ELSE_code = javascript.javascriptGenerator.statementToCode(block, 'ELSE');
  var ELSE_stmt = (ELSE_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "maze_ifElse", "_blockId": ' + JSON.stringify(block.id) +  ', "DIR": ' + JSON.stringify(DIR) +   ', "DO": ' + (DO_stmt ? '[' + DO_stmt + ']' : '[]') +   ', "ELSE": ' + (ELSE_stmt ? '[' + ELSE_stmt + ']' : '[]') + ' },\n';
};
