...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'maze_forever') {
			...
			if (count > 10)
				queueWarn(block, '"maze_forever.DO" allows at most 10 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'maze_if') {
			var count = 0;
			var child = block.getInputTargetBlock('DO');
			while (child) { count++; child = child.getNextBlock(); }
			if (count > 10)
				queueWarn(block, '"maze_if.DO" allows at most 10 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'maze_ifElse') {
			var count = 0;
			var child = block.getInputTargetBlock('DO');
			while (child) { count++; child = child.getNextBlock(); }
			if (count > 10)
				queueWarn(block, '"maze_ifElse.DO" allows at most 10 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'maze_ifElse') {
			var count = 0;
			var child = block.getInputTargetBlock('ELSE');
			while (child) { count++; child = child.getNextBlock(); }
			if (count > 10)
				queueWarn(block, '"maze_ifElse.ELSE" allows at most 10 element(s). Has: ' + count);
		}
	});
	return warnings;
}
