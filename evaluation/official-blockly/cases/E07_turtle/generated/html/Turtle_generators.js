// Code generators for domain "Turtle".
...
javascript.javascriptGenerator.forBlock['turtle_move'] = function(block) {
  var DIR = block.getFieldValue('DIR');
  var VALUE_code = javascript.javascriptGenerator.valueToCode(block, 'VALUE', 0) || 'null';
  return '{' +   '"_type": "turtle_move", "_blockId": ' + JSON.stringify(block.id) +  ', "DIR": ' + JSON.stringify(DIR) +  ', "VALUE": ' + VALUE_code + ' },\n';
};
javascript.javascriptGenerator.forBlock['turtle_move_internal'] = function(block) {
  var DIR = block.getFieldValue('DIR');
  var VALUE = block.getFieldValue('VALUE');
  return '{' +   '"_type": "turtle_move_internal", "_blockId": ' + JSON.stringify(block.id) +  ', "DIR": ' + JSON.stringify(DIR) +  ', "VALUE": ' + JSON.stringify(VALUE) + ' },\n';
};
javascript.javascriptGenerator.forBlock['turtle_turn'] = function(block) {
  var DIR = block.getFieldValue('DIR');
  var VALUE_code = javascript.javascriptGenerator.valueToCode(block, 'VALUE', 0) || 'null';
  return '{' +   '"_type": "turtle_turn", "_blockId": ' + JSON.stringify(block.id) +  ', "DIR": ' + JSON.stringify(DIR) +  ', "VALUE": ' + VALUE_code + ' },\n';
};
javascript.javascriptGenerator.forBlock['turtle_turn_internal'] = function(block) {
  var DIR = block.getFieldValue('DIR');
  var VALUE = block.getFieldValue('VALUE');
  return '{' +   '"_type": "turtle_turn_internal", "_blockId": ' + JSON.stringify(block.id) +  ', "DIR": ' + JSON.stringify(DIR) +  ', "VALUE": ' + JSON.stringify(VALUE) + ' },\n';
};
javascript.javascriptGenerator.forBlock['turtle_width'] = function(block) {
  var WIDTH_code = javascript.javascriptGenerator.valueToCode(block, 'WIDTH', 0) || 'null';
  return '{' +   '"_type": "turtle_width", "_blockId": ' + JSON.stringify(block.id) +  ', "WIDTH": ' + WIDTH_code + ' },\n';
};
javascript.javascriptGenerator.forBlock['turtle_pen'] = function(block) {
  var PEN = block.getFieldValue('PEN');
  return '{' +   '"_type": "turtle_pen", "_blockId": ' + JSON.stringify(block.id) +  ', "PEN": ' + JSON.stringify(PEN) + ' },\n';
};
javascript.javascriptGenerator.forBlock['turtle_colour'] = function(block) {
  var COLOUR_code = javascript.javascriptGenerator.valueToCode(block, 'COLOUR', 0) || 'null';
  return '{' +   '"_type": "turtle_colour", "_blockId": ' + JSON.stringify(block.id) +  ', "COLOUR": ' + COLOUR_code + ' },\n';
};
javascript.javascriptGenerator.forBlock['turtle_colour_internal'] = function(block) {
  var COLOUR = block.getFieldValue('COLOUR');
  return '{' +   '"_type": "turtle_colour_internal", "_blockId": ' + JSON.stringify(block.id) +  ', "COLOUR": ' + JSON.stringify(COLOUR) + ' },\n';
};
javascript.javascriptGenerator.forBlock['turtle_visibility'] = function(block) {
  var VISIBILITY = block.getFieldValue('VISIBILITY');
  return '{' +   '"_type": "turtle_visibility", "_blockId": ' + JSON.stringify(block.id) +  ', "VISIBILITY": ' + JSON.stringify(VISIBILITY) + ' },\n';
};
javascript.javascriptGenerator.forBlock['turtle_print'] = function(block) {
  var TEXT_code = javascript.javascriptGenerator.valueToCode(block, 'TEXT', 0) || 'null';
  return '{' +   '"_type": "turtle_print", "_blockId": ' + JSON.stringify(block.id) +  ', "TEXT": ' + TEXT_code + ' },\n';
};
javascript.javascriptGenerator.forBlock['turtle_font'] = function(block) {
  var FONT = block.getFieldValue('FONT');
  var FONTSIZE = block.getFieldValue('FONTSIZE');
  var FONTSTYLE = block.getFieldValue('FONTSTYLE');
  return '{' +   '"_type": "turtle_font", "_blockId": ' + JSON.stringify(block.id) +  ', "FONT": ' + JSON.stringify(FONT) +  ', "FONTSIZE": ' + JSON.stringify(FONTSIZE) +  ', "FONTSTYLE": ' + JSON.stringify(FONTSTYLE) + ' },\n';
};
javascript.javascriptGenerator.forBlock['turtle_repeat_internal'] = function(block) {
  var TIMES = block.getFieldValue('TIMES');
  var DO_code = javascript.javascriptGenerator.statementToCode(block, 'DO');
  var DO_stmt = (DO_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "turtle_repeat_internal", "_blockId": ' + JSON.stringify(block.id) +  ', "TIMES": ' + JSON.stringify(TIMES) +   ', "DO": ' + (DO_stmt ? '[' + DO_stmt + ']' : '[]') + ' },\n';
};
...
'turtle_move': {
  label: 'move',
  template: '{{DIR}}({{value:VALUE}});',
  fields: ['DIR'],
  fieldTypes: {'DIR': 'DROPDOWN'},
  references: [],
  values: ['VALUE'],
  statements: []
},
'turtle_move_internal': {
  label: 'move with fixed distance',
  template: '{{DIR}}({{VALUE}});',
  fields: ['DIR', 'VALUE'],
  fieldTypes: {'DIR': 'DROPDOWN', 'VALUE': 'DROPDOWN'},
  references: [],
  values: [],
  statements: []
},
...
'turtle_colour_internal': {
  label: 'set fixed colour',
  template: 'penColour(\'{{COLOUR}}\');',
  fields: ['COLOUR'],
  fieldTypes: {'COLOUR': 'COLOUR'},
  references: [],
  values: [],
  statements: []
},
'turtle_visibility': {
  label: 'turtle visibility',
  template: '{{VISIBILITY}}();',
  fields: ['VISIBILITY'],
  fieldTypes: {'VISIBILITY': 'DROPDOWN'},
  references: [],
  values: [],
  statements: []
},
...
'turtle_font': {
  label: 'font',
  template: 'font(\'{{FONT}}\', {{FONTSIZE}}, \'{{FONTSTYLE}}\');',
  fields: ['FONT', 'FONTSIZE', 'FONTSTYLE'],
  fieldTypes: {'FONT': 'DROPDOWN', 'FONTSIZE': 'FLOAT', 'FONTSTYLE': 'DROPDOWN'},
  references: [],
  values: [],
  statements: []
},
'turtle_repeat_internal': {
  label: 'repeat',
  template: 'for (let count = 0; count < {{TIMES}}; count++) {\n{{statements:DO}}\n}',
  fields: ['TIMES'],
  fieldTypes: {'TIMES': 'DROPDOWN'},
  references: [],
  values: [],
  statements: ['DO']
}
