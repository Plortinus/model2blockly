...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'controls_if') {
			var count = 0;
			var child = block.getInputTargetBlock('DO0');
			while (child) { count++; child = child.getNextBlock(); }
			if (count > 50)
				queueWarn(block, '"controls_if.DO0" allows at most 50 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'controls_whileUntil') {
			...
			while (child) { count++; child = child.getNextBlock(); }
			if (count > 50)
				queueWarn(block, '"controls_whileUntil.DO" allows at most 50 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'procedures_defnoreturn') {
			var count = 0;
			var child = block.getInputTargetBlock('STACK');
			while (child) { count++; child = child.getNextBlock(); }
			if (count > 50)
				queueWarn(block, '"procedures_defnoreturn.STACK" allows at most 50 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'procedures_defreturn') {
			...
			while (child) { count++; child = child.getNextBlock(); }
			if (count > 50)
				queueWarn(block, '"procedures_defreturn.STACK" allows at most 50 element(s). Has: ' + count);
		}
	});
	return warnings;
}
