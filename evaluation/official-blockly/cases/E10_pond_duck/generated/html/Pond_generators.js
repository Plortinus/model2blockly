// Code generators for domain "Pond".
...
javascript.javascriptGenerator.forBlock['pond_scan'] = function(block) {
  var CALL = block.getFieldValue('CALL');
  var DEGREE_code = javascript.javascriptGenerator.valueToCode(block, 'DEGREE', 0) || 'null';
  var code = '{' +   '"_type": "pond_scan", "_blockId": ' + JSON.stringify(block.id) +  ', "CALL": ' + JSON.stringify(CALL) +  ', "DEGREE": ' + DEGREE_code + ' }';
  return [code, 0];
};
javascript.javascriptGenerator.forBlock['pond_cannon'] = function(block) {
  var CALL = block.getFieldValue('CALL');
  var DEGREE_code = javascript.javascriptGenerator.valueToCode(block, 'DEGREE', 0) || 'null';
  var RANGE_code = javascript.javascriptGenerator.valueToCode(block, 'RANGE', 0) || 'null';
  return '{' +   '"_type": "pond_cannon", "_blockId": ' + JSON.stringify(block.id) +  ', "CALL": ' + JSON.stringify(CALL) +  ', "DEGREE": ' + DEGREE_code +  ', "RANGE": ' + RANGE_code + ' },\n';
};
javascript.javascriptGenerator.forBlock['pond_swim'] = function(block) {
  var CALL = block.getFieldValue('CALL');
  var DEGREE_code = javascript.javascriptGenerator.valueToCode(block, 'DEGREE', 0) || 'null';
  return '{' +   '"_type": "pond_swim", "_blockId": ' + JSON.stringify(block.id) +  ', "CALL": ' + JSON.stringify(CALL) +  ', "DEGREE": ' + DEGREE_code + ' },\n';
};
javascript.javascriptGenerator.forBlock['pond_stop'] = function(block) {
  var CALL = block.getFieldValue('CALL');
  var EMPTY = block.getFieldValue('EMPTY');
  return '{' +   '"_type": "pond_stop", "_blockId": ' + JSON.stringify(block.id) +  ', "CALL": ' + JSON.stringify(CALL) +  ', "EMPTY": ' + JSON.stringify(EMPTY) + ' },\n';
};
javascript.javascriptGenerator.forBlock['pond_health'] = function(block) {
  var CALL = block.getFieldValue('CALL');
  var EMPTY = block.getFieldValue('EMPTY');
  var code = '{' +   '"_type": "pond_health", "_blockId": ' + JSON.stringify(block.id) +  ', "CALL": ' + JSON.stringify(CALL) +  ', "EMPTY": ' + JSON.stringify(EMPTY) + ' }';
  return [code, 0];
};
javascript.javascriptGenerator.forBlock['pond_speed'] = function(block) {
  var CALL = block.getFieldValue('CALL');
  var EMPTY = block.getFieldValue('EMPTY');
  var code = '{' +   '"_type": "pond_speed", "_blockId": ' + JSON.stringify(block.id) +  ', "CALL": ' + JSON.stringify(CALL) +  ', "EMPTY": ' + JSON.stringify(EMPTY) + ' }';
  return [code, 0];
};
javascript.javascriptGenerator.forBlock['pond_getX'] = function(block) {
  var CALL = block.getFieldValue('CALL');
  var EMPTY = block.getFieldValue('EMPTY');
  var code = '{' +   '"_type": "pond_getX", "_blockId": ' + JSON.stringify(block.id) +  ', "CALL": ' + JSON.stringify(CALL) +  ', "EMPTY": ' + JSON.stringify(EMPTY) + ' }';
  return [code, 0];
};
javascript.javascriptGenerator.forBlock['pond_getY'] = function(block) {
  var CALL = block.getFieldValue('CALL');
  var EMPTY = block.getFieldValue('EMPTY');
  var code = '{' +   '"_type": "pond_getY", "_blockId": ' + JSON.stringify(block.id) +  ', "CALL": ' + JSON.stringify(CALL) +  ', "EMPTY": ' + JSON.stringify(EMPTY) + ' }';
  return [code, 0];
};
javascript.javascriptGenerator.forBlock['pond_log'] = function(block) {
  var CALL = block.getFieldValue('CALL');
  var VALUE_code = javascript.javascriptGenerator.valueToCode(block, 'VALUE', 0) || 'null';
  return '{' +   '"_type": "pond_log", "_blockId": ' + JSON.stringify(block.id) +  ', "CALL": ' + JSON.stringify(CALL) +  ', "VALUE": ' + VALUE_code + ' },\n';
};
javascript.javascriptGenerator.forBlock['controls_if'] = function(block) {
  var IF0_code = javascript.javascriptGenerator.valueToCode(block, 'IF0', 0) || 'null';
  var DO0_code = javascript.javascriptGenerator.statementToCode(block, 'DO0');
  var DO0_stmt = (DO0_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "controls_if", "_blockId": ' + JSON.stringify(block.id) +  ', "IF0": ' + IF0_code +   ', "DO0": ' + (DO0_stmt ? '[' + DO0_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['logic_compare'] = function(block) {
  var OP = block.getFieldValue('OP');
  var A_code = javascript.javascriptGenerator.valueToCode(block, 'A', 0) || 'null';
  var B_code = javascript.javascriptGenerator.valueToCode(block, 'B', 0) || 'null';
  var code = '{' +   '"_type": "logic_compare", "_blockId": ' + JSON.stringify(block.id) +  ', "OP": ' + JSON.stringify(OP) +  ', "A": ' + A_code +  ', "B": ' + B_code + ' }';
  return [code, 0];
};
javascript.javascriptGenerator.forBlock['logic_operation'] = function(block) {
  var OP = block.getFieldValue('OP');
  var A_code = javascript.javascriptGenerator.valueToCode(block, 'A', 0) || 'null';
  var B_code = javascript.javascriptGenerator.valueToCode(block, 'B', 0) || 'null';
  var code = '{' +   '"_type": "logic_operation", "_blockId": ' + JSON.stringify(block.id) +  ', "OP": ' + JSON.stringify(OP) +  ', "A": ' + A_code +  ', "B": ' + B_code + ' }';
  return [code, 0];
};
javascript.javascriptGenerator.forBlock['logic_boolean'] = function(block) {
  var BOOL = block.getFieldValue('BOOL');
  var code = '{' +   '"_type": "logic_boolean", "_blockId": ' + JSON.stringify(block.id) +  ', "BOOL": ' + JSON.stringify(BOOL) + ' }';
  return [code, 0];
};
javascript.javascriptGenerator.forBlock['controls_whileUntil'] = function(block) {
  var BOOL_code = javascript.javascriptGenerator.valueToCode(block, 'BOOL', 0) || 'null';
  var DO_code = javascript.javascriptGenerator.statementToCode(block, 'DO');
  var DO_stmt = (DO_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "controls_whileUntil", "_blockId": ' + JSON.stringify(block.id) +  ', "BOOL": ' + BOOL_code +   ', "DO": ' + (DO_stmt ? '[' + DO_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['pond_math_number'] = function(block) {
  var NUM = block.getFieldValue('NUM');
  var code = '{' +   '"_type": "pond_math_number", "_blockId": ' + JSON.stringify(block.id) +  ', "NUM": ' + JSON.stringify(NUM) + ' }';
  return [code, 0];
};
javascript.javascriptGenerator.forBlock['math_arithmetic'] = function(block) {
  var OP = block.getFieldValue('OP');
  var A_code = javascript.javascriptGenerator.valueToCode(block, 'A', 0) || 'null';
  var B_code = javascript.javascriptGenerator.valueToCode(block, 'B', 0) || 'null';
  var code = '{' +   '"_type": "math_arithmetic", "_blockId": ' + JSON.stringify(block.id) +  ', "OP": ' + JSON.stringify(OP) +  ', "A": ' + A_code +  ', "B": ' + B_code + ' }';
  return [code, 0];
};
javascript.javascriptGenerator.forBlock['pond_math_single'] = function(block) {
  var OP = block.getFieldValue('OP');
  var NUM_code = javascript.javascriptGenerator.valueToCode(block, 'NUM', 0) || 'null';
  var code = '{' +   '"_type": "pond_math_single", "_blockId": ' + JSON.stringify(block.id) +  ', "OP": ' + JSON.stringify(OP) +  ', "NUM": ' + NUM_code + ' }';
  return [code, 0];
};
javascript.javascriptGenerator.forBlock['math_random_float'] = function(block) {
  var code = '{' +   '"_type": "math_random_float", "_blockId": ' + JSON.stringify(block.id) + ' }';
...
javascript.javascriptGenerator.forBlock['math_change'] = function(block) {
  var VAR = block.getFieldValue('VAR');
  var DELTA_code = javascript.javascriptGenerator.valueToCode(block, 'DELTA', 0) || 'null';
  return '{' +   '"_type": "math_change", "_blockId": ' + JSON.stringify(block.id) +  ', "VAR": ' + JSON.stringify(VAR) +  ', "DELTA": ' + DELTA_code + ' },\n';
};
javascript.javascriptGenerator.forBlock['variables_set'] = function(block) {
  var VAR = block.getFieldValue('VAR');
  var VALUE_code = javascript.javascriptGenerator.valueToCode(block, 'VALUE', 0) || 'null';
  return '{' +   '"_type": "variables_set", "_blockId": ' + JSON.stringify(block.id) +  ', "VAR": ' + JSON.stringify(VAR) +  ', "VALUE": ' + VALUE_code + ' },\n';
};
javascript.javascriptGenerator.forBlock['procedures_defnoreturn'] = function(block) {
  var FUNCTION = block.getFieldValue('FUNCTION');
  var NAME = block.getFieldValue('NAME');
  var OPEN = block.getFieldValue('OPEN');
  var PARAMS = block.getFieldValue('PARAMS');
  var BODY = block.getFieldValue('BODY');
  var CLOSE = block.getFieldValue('CLOSE');
  var STACK_code = javascript.javascriptGenerator.statementToCode(block, 'STACK');
  var STACK_stmt = (STACK_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "procedures_defnoreturn", "_blockId": ' + JSON.stringify(block.id) +  ', "FUNCTION": ' + JSON.stringify(FUNCTION) +  ', "NAME": ' + JSON.stringify(NAME) +  ', "OPEN": ' + JSON.stringify(OPEN) +  ', "PARAMS": ' + JSON.stringify(PARAMS) +  ', "BODY": ' + JSON.stringify(BODY) +  ', "CLOSE": ' + JSON.stringify(CLOSE) +   ', "STACK": ' + (STACK_stmt ? '[' + STACK_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['procedures_defreturn'] = function(block) {
  var FUNCTION = block.getFieldValue('FUNCTION');
  var NAME = block.getFieldValue('NAME');
  ...
  var BODY = block.getFieldValue('BODY');
  var CLOSE = block.getFieldValue('CLOSE');
  var RETURN_code = javascript.javascriptGenerator.valueToCode(block, 'RETURN', 0) || 'null';
  var STACK_code = javascript.javascriptGenerator.statementToCode(block, 'STACK');
  var STACK_stmt = (STACK_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "procedures_defreturn", "_blockId": ' + JSON.stringify(block.id) +  ', "FUNCTION": ' + JSON.stringify(FUNCTION) +  ', "NAME": ' + JSON.stringify(NAME) +  ', "OPEN": ' + JSON.stringify(OPEN) +  ', "PARAMS": ' + JSON.stringify(PARAMS) +  ', "BODY": ' + JSON.stringify(BODY) +  ', "CLOSE": ' + JSON.stringify(CLOSE) +  ', "RETURN": ' + RETURN_code +   ', "STACK": ' + (STACK_stmt ? '[' + STACK_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['procedures_callnoreturn'] = function(block) {
  var NAME = block.getFieldValue('NAME');
  var OPEN = block.getFieldValue('OPEN');
  var TAIL = block.getFieldValue('TAIL');
  return '{' +   '"_type": "procedures_callnoreturn", "_blockId": ' + JSON.stringify(block.id) +  ', "NAME": ' + JSON.stringify(NAME) +  ', "OPEN": ' + JSON.stringify(OPEN) +  ', "TAIL": ' + JSON.stringify(TAIL) + ' },\n';
};
javascript.javascriptGenerator.forBlock['procedures_callreturn'] = function(block) {
  var NAME = block.getFieldValue('NAME');
  var OPEN = block.getFieldValue('OPEN');
  var TAIL = block.getFieldValue('TAIL');
  var code = '{' +   '"_type": "procedures_callreturn", "_blockId": ' + JSON.stringify(block.id) +  ', "NAME": ' + JSON.stringify(NAME) +  ', "OPEN": ' + JSON.stringify(OPEN) +  ', "TAIL": ' + JSON.stringify(TAIL) + ' }';
  return [code, 0];
};
...
'pond_stop': {
  ...
  template: 'stop();',
  fields: ['CALL', 'EMPTY'],
  fieldTypes: {'CALL': 'LABEL', 'EMPTY': 'LABEL'},
  references: [],
  values: [],
  statements: []
},
...
'logic_compare': {
  label: 'compare',
  template: '{{value:A}} {{OP}} {{value:B}}',
  fields: ['OP'],
  fieldTypes: {'OP': 'DROPDOWN'},
  references: [],
  values: ['A', 'B'],
  statements: []
},
...
'procedures_defnoreturn': {
  ...
  fields: ['FUNCTION', 'NAME', 'OPEN', 'PARAMS', 'BODY', 'CLOSE'],
  fieldTypes: {'FUNCTION': 'LABEL', 'NAME': 'TEXT', 'OPEN': 'LABEL', 'PARAMS': 'LABEL', 'BODY': 'LABEL', 'CLOSE': 'LABEL'},
  references: [],
  values: [],
  statements: ['STACK']
},
'procedures_defreturn': {
  label: 'function definition with return',
  template: 'function {{NAME}}({{PARAMS}}) {\n{{statements:STACK}}\nreturn {{value:RETURN}};\n}',
  fields: ['FUNCTION', 'NAME', 'OPEN', 'PARAMS', 'BODY', 'CLOSE'],
  fieldTypes: {'FUNCTION': 'LABEL', 'NAME': 'TEXT', 'OPEN': 'LABEL', 'PARAMS': 'LABEL', 'BODY': 'LABEL', 'CLOSE': 'LABEL'},
  references: [],
...
'procedures_callnoreturn': {
  label: 'function call',
  template: '{{NAME}}();',
  fields: ['NAME', 'OPEN', 'TAIL'],
  fieldTypes: {'NAME': 'LABEL', 'OPEN': 'LABEL', 'TAIL': 'LABEL'},
  references: [],
  values: [],
  statements: []
},
'procedures_callreturn': {
  label: 'function call with return',
  template: '{{NAME}}()',
  fields: ['NAME', 'OPEN', 'TAIL'],
  fieldTypes: {'NAME': 'LABEL', 'OPEN': 'LABEL', 'TAIL': 'LABEL'},
  references: [],
