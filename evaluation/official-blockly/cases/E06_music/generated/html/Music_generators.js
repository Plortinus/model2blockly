// Code generators for domain "Music".
...
javascript.javascriptGenerator.forBlock['music_pitch'] = function(block) {
  var PITCH = block.getFieldValue('PITCH');
  var code = '{' +   '"_type": "music_pitch", "_blockId": ' + JSON.stringify(block.id) +  ', "PITCH": ' + JSON.stringify(PITCH) + ' }';
  return [code, 0];
};
javascript.javascriptGenerator.forBlock['music_note'] = function(block) {
  var DURATION = block.getFieldValue('DURATION');
  var PITCH_code = javascript.javascriptGenerator.valueToCode(block, 'PITCH', 0) || 'null';
  return '{' +   '"_type": "music_note", "_blockId": ' + JSON.stringify(block.id) +  ', "DURATION": ' + JSON.stringify(DURATION) +  ', "PITCH": ' + PITCH_code + ' },\n';
};
javascript.javascriptGenerator.forBlock['music_rest_whole'] = function(block) {
  var REST_IMAGE = block.getFieldValue('REST_IMAGE');
  return '{' +   '"_type": "music_rest_whole", "_blockId": ' + JSON.stringify(block.id) +  ', "REST_IMAGE": ' + JSON.stringify(REST_IMAGE) + ' },\n';
};
javascript.javascriptGenerator.forBlock['music_rest'] = function(block) {
  var DURATION = block.getFieldValue('DURATION');
  return '{' +   '"_type": "music_rest", "_blockId": ' + JSON.stringify(block.id) +  ', "DURATION": ' + JSON.stringify(DURATION) + ' },\n';
};
javascript.javascriptGenerator.forBlock['music_instrument'] = function(block) {
  var INSTRUMENT = block.getFieldValue('INSTRUMENT');
  return '{' +   '"_type": "music_instrument", "_blockId": ' + JSON.stringify(block.id) +  ', "INSTRUMENT": ' + JSON.stringify(INSTRUMENT) + ' },\n';
};
javascript.javascriptGenerator.forBlock['music_start'] = function(block) {
  var PLAY_IMAGE = block.getFieldValue('PLAY_IMAGE');
  var STACK_code = javascript.javascriptGenerator.statementToCode(block, 'STACK');
  var STACK_stmt = (STACK_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "music_start", "_blockId": ' + JSON.stringify(block.id) +  ', "PLAY_IMAGE": ' + JSON.stringify(PLAY_IMAGE) +   ', "STACK": ' + (STACK_stmt ? '[' + STACK_stmt + ']' : '[]') + ' },\n';
};
...
'music_note': {
  label: 'play note',
  template: 'play({{DURATION}}, {{value:PITCH}});',
  fields: ['DURATION'],
  fieldTypes: {'DURATION': 'DROPDOWN'},
  references: [],
  values: ['PITCH'],
  statements: []
},
...
'music_instrument': {
  ...
  fields: ['INSTRUMENT'],
  fieldTypes: {'INSTRUMENT': 'DROPDOWN'},
  references: [],
  values: [],
  statements: []
},
