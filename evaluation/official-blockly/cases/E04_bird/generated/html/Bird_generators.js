// Code generators for domain "Bird".
...
javascript.javascriptGenerator.forBlock['bird_noWorm'] = function(block) {
  var code = '{' +   '"_type": "bird_noWorm", "_blockId": ' + JSON.stringify(block.id) + ' }';
  return [code, 0];
};
javascript.javascriptGenerator.forBlock['bird_heading'] = function(block) {
  var ANGLE = block.getFieldValue('ANGLE');
  return '{' +   '"_type": "bird_heading", "_blockId": ' + JSON.stringify(block.id) +  ', "ANGLE": ' + JSON.stringify(ANGLE) + ' },\n';
};
javascript.javascriptGenerator.forBlock['bird_position'] = function(block) {
  var XY = block.getFieldValue('XY');
  var code = '{' +   '"_type": "bird_position", "_blockId": ' + JSON.stringify(block.id) +  ', "XY": ' + JSON.stringify(XY) + ' }';
  return [code, 0];
};
javascript.javascriptGenerator.forBlock['bird_compare'] = function(block) {
  ...
  var code = '{' +   '"_type": "bird_compare", "_blockId": ' + JSON.stringify(block.id) +  ', "OP": ' + JSON.stringify(OP) +  ', "A": ' + A_code +  ', "B": ' + B_code + ' }';
  return [code, 0];
};
javascript.javascriptGenerator.forBlock['bird_and'] = function(block) {
  var A_code = javascript.javascriptGenerator.valueToCode(block, 'A', 0) || 'null';
  var B_code = javascript.javascriptGenerator.valueToCode(block, 'B', 0) || 'null';
  var code = '{' +   '"_type": "bird_and", "_blockId": ' + JSON.stringify(block.id) +  ', "A": ' + A_code +  ', "B": ' + B_code + ' }';
  return [code, 0];
};
javascript.javascriptGenerator.forBlock['bird_ifElse'] = function(block) {
  var CONDITION_code = javascript.javascriptGenerator.valueToCode(block, 'CONDITION', 0) || 'null';
  var DO_code = javascript.javascriptGenerator.statementToCode(block, 'DO');
  var DO_stmt = (DO_code || '').trim().replace(/,\\s*$/, '');
  var ELSE_code = javascript.javascriptGenerator.statementToCode(block, 'ELSE');
  var ELSE_stmt = (ELSE_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "bird_ifElse", "_blockId": ' + JSON.stringify(block.id) +  ', "CONDITION": ' + CONDITION_code +   ', "DO": ' + (DO_stmt ? '[' + DO_stmt + ']' : '[]') +   ', "ELSE": ' + (ELSE_stmt ? '[' + ELSE_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['math_number'] = function(block) {
  var NUM = block.getFieldValue('NUM');
  var code = '{' +   '"_type": "math_number", "_blockId": ' + JSON.stringify(block.id) +  ', "NUM": ' + JSON.stringify(NUM) + ' }';
  return [code, 0];
};
...
'bird_heading': {
  label: 'heading',
  template: 'heading({{ANGLE}});',
  fields: ['ANGLE'],
  fieldTypes: {'ANGLE': 'ANGLE'},
  references: [],
  values: [],
  statements: []
},
