...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'bird_ifElse') {
			var count = 0;
			var child = block.getInputTargetBlock('DO');
			while (child) { count++; child = child.getNextBlock(); }
			if (count > 10)
				queueWarn(block, '"bird_ifElse.DO" allows at most 10 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'bird_ifElse') {
			...
			while (child) { count++; child = child.getNextBlock(); }
			if (count > 10)
				queueWarn(block, '"bird_ifElse.ELSE" allows at most 10 element(s). Has: ' + count);
		}
	});
	return warnings;
}
