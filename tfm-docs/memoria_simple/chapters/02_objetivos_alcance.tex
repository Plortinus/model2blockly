...
específicos, identificados en adelante como OE1--OE7:
...
sintaxis visual de bloques. El estado del espacio de trabajo puede guardarse en JSON y
...
\medskip
\noindent\textbf{OE7. Evaluar la efectividad, el esfuerzo de especificación y
los límites de la solución.} Contrastar Model2Blockly con editores Blockly
existentes mediante propiedades observables, generadores y tamaño de la fuente
mantenida. Comprobar en una muestra si Ecore y \texttt{.m2b} convergen en el
mismo descriptor. Los seis pares mínimos y complementarios, el caso Ecore
específico y las pruebas automáticas aportan evidencia complementaria de
integración, regresión y despliegue.
...
\begin{table}[h]
  ...
  \begin{tabular}{|p{0.32\textwidth}|p{0.56\textwidth}|}
    ...
    independiente de la ruta de entrada. \\
	    Generación Blockly & Producción de bloques, paleta, generadores,
	    validaciones y páginas HTML ejecutables a partir de la representación
	    intermedia. \\
	    Integración Eclipse & Complementos Eclipse/Xtext para editar ficheros
	    \texttt{.m2b}, ejecutar la generación desde el menú contextual y
	    publicar la herramienta como feature instalable desde un sitio de actualización p2. \\
	    Artefactos trazables & Generación del XMI intermedio, informe de generación,
	    modelo de ejemplo, espacio de trabajo visual de validaciones y documentación breve de
	    la salida producida. \\
    ...
    Evaluación & Comparación con editores Blockly existentes: paridad funcional,
    fuente mantenida y rutas Ecore/\texttt{.m2b}; evidencia complementaria con
    seis pares, un caso Ecore específico y pruebas automáticas. \\
    \hline
  \end{tabular}
...
\begin{itemize}
  \item la importación genérica de modelos XMI existentes a un espacio de trabajo
  Blockly;
...
\begin{enumerate}
  ...
  sin editar directamente Ecore en casos sencillos o ejemplos;
	  ...
	  \item comparar con un corpus de editores Blockly existentes qué propiedades
	  pueden reproducirse, qué tamaño tiene la fuente mantenida y qué capacidades
	  quedan fuera del soporte actual;
	  \item comprobar la coherencia de las rutas Ecore y \texttt{.m2b}, y disponer
	  de pruebas, verificaciones y un mecanismo de instalación que reduzcan el
	  riesgo de que la solución funcione únicamente para un caso preparado
	  manualmente.
\end{enumerate}
