...
encaje, toolbox y serialización \cite{blocklyWhatIs,blocklyDocs}.
...
workspace. El Listado \ref{lst:blockly-infraestructura-minima} resume esta
infraestructura mínima siguiendo el codelab oficial
\cite{blocklyGettingStartedCodelab}.

\begin{lstlisting}[caption={Infraestructura mínima para crear un workspace Blockly}, label={lst:blockly-infraestructura-minima}]
...
posterior. El segundo determina qué tipos de bloque aparecen en la toolbox.
...
\begin{lstlisting}[caption={Definición de un bloque y configuración mínima de una toolbox en Blockly}, label={lst:blockly-music-maker-config}]
...
toolbox ofrece el bloque de repetición y el bloque musical, que pueden
arrastrarse al workspace para construir el programa.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.48\textwidth]{assets/blockly/workspace_with_toolbox.png}
  \caption{Workspace y toolbox obtenidos tras registrar el bloque
...
el workspace y declarar manualmente la estructura, los campos, las conexiones y
la toolbox. Todavía habría que añadir el generador de código y la lógica de
...
organización de la toolbox son decisiones de sintaxis concreta. Por ello, el
...
\begin{lstlisting}[caption={Ejemplo breve de una definición textual \texttt{.m2b}}, label={lst:xtext-m2b-breve}]
domain Appmaker
...
La declaración \texttt{class Page} representa un concepto que podrá convertirse
en un tipo de bloque; \texttt{title} aporta una propiedad editable y
\texttt{components} expresa una relación jerárquica con cardinalidad. Xtext
comprueba que la entrada respeta la gramática y que nombres como
\texttt{Pages} y \texttt{Component} se pueden resolver. El resultado es un
...
widgets, organización de la toolbox y plantillas de código. La convergencia en
...
\begin{lstlisting}[caption={Ejemplo de una plantilla Xtend usada para generar JavaScript}, label={lst:xtend-plantilla}]
...
plantillas que producen bloques, toolbox, validaciones y páginas HTML. Gracias
...
\begin{table}[h]
  ...
  \begin{tabular}{|L{0.24\textwidth}|L{0.30\textwidth}|L{0.34\textwidth}|}
    ...
    ejecutables en la web. & Requiere codificar bloques, toolbox, conexiones,
    ...
    contrato XMI explícito para la generación. \\
	    ...
	    intermedio y generación. & Su validación y evaluación se
    limitan al conjunto de casos y reglas estructurales estudiados. \\
    \hline
  \end{tabular}
...
\begin{table}[h]
  ...
  \begin{tabular}{|p{0.22\textwidth}|p{0.66\textwidth}|}
    ...
    \hline
    Blockly & Plataforma web para representar bloques, toolbox, workspace,
