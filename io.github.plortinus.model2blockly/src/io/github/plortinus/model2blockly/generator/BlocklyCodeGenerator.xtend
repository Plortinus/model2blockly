...
class BlocklyCodeGenerator {
	...
	def String fieldToArg(FieldSpec field) {
		if (field.many) {
			val options = field.options.map[
				'''[«toJsonString(label ?: value ?: "")», «toJsonString(value ?: "")»]'''
			].join(", ")
			return '''{"type": "field_multivalue", "name": "«field.name»", "label": «toJsonString(field.uiLabel ?: field.name)», "text": «toJsonString(field.defaultValue ?: "")», "itemType": «toJsonString(field.fieldType.toString)», "options": [«options»], "min": «toJsonString(field.min)», "max": «toJsonString(field.max)», "lowerBound": «field.lowerBound», "upperBound": «field.upperBound», "unique": «field.unique», "ordered": «field.ordered»}'''
		}
	...
	def String blockTypeToGeneratorJs(BlockTypeSpec bt) {
		val fieldReads = bt.fields.map[
			if (it.many)
				'''  var «it.name» = parseBlocklyTypedListField(block.getFieldValue('«it.name»'), «toJsonString(it.fieldType.toString)»);'''
			else
		...
		'''
			<style>
				...
				*{box-sizing:border-box}
				...
				.multivalue-list{display:flex;flex-direction:column;gap:6px}
				.multivalue-row{padding:0}
				.multivalue-control{box-sizing:border-box;flex:1;min-width:0;border:1px solid #dadce0;border-radius:4px;padding:7px 8px;background:#fff;color:#202124}
				.multivalue-control[type="color"]{min-height:36px;padding:3px}
				.multivalue-icon-button{width:32px;height:32px;border:1px solid #dadce0;border-radius:4px;background:#fff;cursor:pointer;font-size:16px;line-height:1}
				.multivalue-errors{min-height:0;margin:0 16px;color:#b3261e;font-size:.82em;line-height:1.4}
				.multivalue-errors:not(:empty){padding:8px 0}
				.multivalue-hint{padding:6px 16px 10px;color:#666;font-size:.8em}
