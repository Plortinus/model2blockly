// Code generators for domain "Relations".
...
javascript.javascriptGenerator.forBlock['Workflow'] = function(block) {
  var name = block.getFieldValue('name');
  var elements_code = javascript.javascriptGenerator.statementToCode(block, 'elements');
  var elements_stmt = (elements_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "Workflow", "_blockId": ' + JSON.stringify(block.id) +  ', "name": ' + JSON.stringify(name) +   ', "elements": ' + (elements_stmt ? '[' + elements_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['Card'] = function(block) {
  var cardId = block.getFieldValue('cardId');
  var title = block.getFieldValue('title');
  var outgoing = parseBlocklyListField(block.getFieldValue('outgoing'));
  var content_code = javascript.javascriptGenerator.valueToCode(block, 'content', 0) || 'null';
  return '{' +   '"_type": "Card", "_blockId": ' + JSON.stringify(block.id) +  ', "_modelId": ' + JSON.stringify(cardId || block.id) +  ', "cardId": ' + JSON.stringify(cardId) +  ', "title": ' + JSON.stringify(title) +  ', "outgoing": ' + JSON.stringify(outgoing) +  ', "content": ' + content_code + ' },\n';
};
javascript.javascriptGenerator.forBlock['Link'] = function(block) {
  var linkId = block.getFieldValue('linkId');
  var description = block.getFieldValue('description');
  var source = block.getFieldValue('source');
  var target = block.getFieldValue('target');
  return '{' +   '"_type": "Link", "_blockId": ' + JSON.stringify(block.id) +  ', "_modelId": ' + JSON.stringify(linkId || block.id) +  ', "linkId": ' + JSON.stringify(linkId) +  ', "description": ' + JSON.stringify(description) +  ', "source": ' + JSON.stringify(source) +  ', "target": ' + JSON.stringify(target) + ' },\n';
};
javascript.javascriptGenerator.forBlock['TextLiteral'] = function(block) {
  var textValue = block.getFieldValue('textValue');
  var code = '{' +   '"_type": "TextLiteral", "_blockId": ' + JSON.stringify(block.id) +  ', "textValue": ' + JSON.stringify(textValue) + ' }';
  return [code, 0];
};
...
'Card': {
  ...
  template: null,
  fields: ['cardId', 'title'],
  fieldTypes: {'cardId': 'TEXT', 'title': 'TEXT'},
  references: ['outgoing'],
  values: ['content'],
  statements: []
},
'Link': {
  label: 'Link',
  template: null,
  fields: ['linkId', 'description'],
  fieldTypes: {'linkId': 'TEXT', 'description': 'TEXT'},
