// Code generators for domain "BasicStructure".
...
javascript.javascriptGenerator.forBlock['Board'] = function(block) {
  var name = block.getFieldValue('name');
  var items_code = javascript.javascriptGenerator.statementToCode(block, 'items');
  var items_stmt = (items_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "Board", "_blockId": ' + JSON.stringify(block.id) +  ', "name": ' + JSON.stringify(name) +   ', "items": ' + (items_stmt ? '[' + items_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['Task'] = function(block) {
  var title = block.getFieldValue('title');
  var priority = block.getFieldValue('priority');
  var done = block.getFieldValue('done');
  return '{' +   '"_type": "Task", "_blockId": ' + JSON.stringify(block.id) +  ', "title": ' + JSON.stringify(title) +  ', "priority": ' + JSON.stringify(priority) +  ', "done": ' + JSON.stringify(done) + ' },\n';
};
javascript.javascriptGenerator.forBlock['Note'] = function(block) {
  var body = block.getFieldValue('body');
  return '{' +   '"_type": "Note", "_blockId": ' + JSON.stringify(block.id) +  ', "body": ' + JSON.stringify(body) + ' },\n';
};
javascript.javascriptGenerator.forBlock['Tag'] = function(block) {
  var name = block.getFieldValue('name');
  return '{' +   '"_type": "Tag", "_blockId": ' + JSON.stringify(block.id) +  ', "name": ' + JSON.stringify(name) + ' },\n';
};
...
'Task': {
  ...
  fieldTypes: {'title': 'TEXT', 'priority': 'DROPDOWN', 'done': 'BOOLEAN'},
  references: [],
  values: [],
  statements: []
},
