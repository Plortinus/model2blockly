...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Board') {
			var count = 0;
			var child = block.getInputTargetBlock('items');
			while (child) { count++; child = child.getNextBlock(); }
			if (count < 1)
				queueWarn(block, '"Board.items" needs at least 1 element(s). Has: ' + count);
			if (count > 4)
				queueWarn(block, '"Board.items" allows at most 4 element(s). Has: ' + count);
		}
	});
	return warnings;
}
