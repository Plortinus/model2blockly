// Code generators for domain "FieldsUi".
...
javascript.javascriptGenerator.forBlock['ContentForm'] = function(block) {
  ...
  var enabled = block.getFieldValue('enabled');
  var confirmed = block.getFieldValue('confirmed');
  var mode = block.getFieldValue('mode');
  var alignment = block.getFieldValue('alignment');
  var tags = parseBlocklyListField(block.getFieldValue('tags'));
  var internalCode = block.getFieldValue('internalCode');
  return '{' +   '"_type": "ContentForm", "_blockId": ' + JSON.stringify(block.id) +  ', "title": ' + JSON.stringify(title) +  ', "description": ' + JSON.stringify(description) +  ', "quantity": ' + JSON.stringify(quantity) +  ', "opacity": ' + JSON.stringify(opacity) +  ', "enabled": ' + JSON.stringify(enabled) +  ', "confirmed": ' + JSON.stringify(confirmed) +  ', "mode": ' + JSON.stringify(mode) +  ', "alignment": ' + JSON.stringify(alignment) +  ', "tags": ' + JSON.stringify(tags) +  ', "internalCode": ' + JSON.stringify(internalCode) + ' },\n';
};
javascript.javascriptGenerator.forBlock['VisualFields'] = function(block) {
  var colourValue = block.getFieldValue('colourValue');
  var angleValue = block.getFieldValue('angleValue');
  var imageValue = block.getFieldValue('imageValue');
  var labelValue = block.getFieldValue('labelValue');
  return '{' +   '"_type": "VisualFields", "_blockId": ' + JSON.stringify(block.id) +  ', "colourValue": ' + JSON.stringify(colourValue) +  ', "angleValue": ' + JSON.stringify(angleValue) +  ', "imageValue": ' + JSON.stringify(imageValue) +  ', "labelValue": ' + JSON.stringify(labelValue) + ' },\n';
};
...
'ContentForm': {
  label: 'Content form',
  template: null,
  fields: ['title', 'description', 'quantity', 'opacity', 'enabled', 'confirmed', 'mode', 'alignment', 'tags', 'internalCode'],
  fieldTypes: {'title': 'TEXT', 'description': 'TEXT', 'quantity': 'INTEGER', 'opacity': 'FLOAT', 'enabled': 'BOOLEAN', 'confirmed': 'BOOLEAN', 'mode': 'DROPDOWN', 'alignment': 'DROPDOWN', 'tags': 'TEXT', 'internalCode': 'TEXT'},
  references: [],
  values: [],
  statements: []
},
'VisualFields': {
  label: 'Visual fields',
  template: null,
  fields: ['colourValue', 'angleValue', 'imageValue', 'labelValue'],
  fieldTypes: {'colourValue': 'COLOUR', 'angleValue': 'ANGLE', 'imageValue': 'IMAGE', 'labelValue': 'LABEL'},
  references: [],
  values: [],
  statements: []
}
