// Code generators for domain "ValidationCodegen".
...
javascript.javascriptGenerator.forBlock['Page'] = function(block) {
  var pageId = block.getFieldValue('pageId');
  var title = block.getFieldValue('title');
  var route = block.getFieldValue('route');
  var actions_code = javascript.javascriptGenerator.statementToCode(block, 'actions');
  var actions_stmt = (actions_code || '').trim().replace(/,\\s*$/, '');
  return '{' +   '"_type": "Page", "_blockId": ' + JSON.stringify(block.id) +  ', "_modelId": ' + JSON.stringify(pageId || block.id) +  ', "pageId": ' + JSON.stringify(pageId) +  ', "title": ' + JSON.stringify(title) +  ', "route": ' + JSON.stringify(route) +   ', "actions": ' + (actions_stmt ? '[' + actions_stmt + ']' : '[]') + ' },\n';
};
javascript.javascriptGenerator.forBlock['Alert'] = function(block) {
  var message = block.getFieldValue('message');
  return '{' +   '"_type": "Alert", "_blockId": ' + JSON.stringify(block.id) +  ', "message": ' + JSON.stringify(message) + ' },\n';
};
...
'Page': {
  label: 'Page',
  template: 'page({{title}}, [{{statements:actions}}]);',
  fields: ['pageId', 'title', 'route'],
  fieldTypes: {'pageId': 'TEXT', 'title': 'TEXT', 'route': 'TEXT'},
  references: [],
  values: [],
  statements: ['actions']
},
