// Validations for domain "ValidationCodegen".
...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Page') {
			var count = 0;
			var child = block.getInputTargetBlock('actions');
			while (child) { count++; child = child.getNextBlock(); }
			if (count < 1)
				queueWarn(block, '"Page.actions" needs at least 1 element(s). Has: ' + count);
			if (count > 4)
				queueWarn(block, '"Page.actions" allows at most 4 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Page') {
			var val = block.getFieldValue('pageId');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"Page.pageId" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Page') {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Alert') {
			...
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"Alert.message" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Navigate') {
	...
	(function() {
		...
		workspace.getAllBlocks(false).forEach(function(block) {
			if (block.type === 'Page') {
				...
				if (!val) return;
				if (seen[val]) {
					queueWarn(block, '"Page.pageId" must be unique. Duplicate ID: ' + val);
					queueWarn(seen[val], '"Page.pageId" must be unique. Duplicate ID: ' + val);
				} else {
					seen[val] = block;
				}
			}
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Page') {
			var result = evaluateValidationExpression(block, "has(\'title\')");
			if (!result.ok) {
				var message = "Page title is required.";
				if (result.error) message += ' Expression error: ' + result.error;
				queueWarn(block, message);
			}
		}
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Page') {
			var result = evaluateValidationExpression(block, "value(\'route\').startsWith(\'/\')");
			if (!result.ok) {
				var message = "Route must start with /.";
				if (result.error) message += ' Expression error: ' + result.error;
				queueWarn(block, message);
			}
		}
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Page') {
			var result = evaluateValidationExpression(block, "size(\'actions\') <= 4");
			if (!result.ok) {
				var message = "A page supports at most four actions.";
				if (result.error) message += ' Expression error: ' + result.error;
				queueWarn(block, message);
			}
		}
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Page') {
			var result = evaluateValidationExpression(block, "size(\"actions\") >= 1");
			if (!result.ok) {
				var message = "Constraint \"HasActions\" is violated on Page.";
				if (result.error) message += ' Expression error: ' + result.error;
				queueWarn(block, message);
			}
		}
