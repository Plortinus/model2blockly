// Code generators for domain "ReferencesIntegrity".
...
javascript.javascriptGenerator.forBlock['Card'] = function(block) {
  ...
  return '{' +   '"_type": "Card", "_blockId": ' + JSON.stringify(block.id) +  ', "_modelId": ' + JSON.stringify(cardId || block.id) +  ', "cardId": ' + JSON.stringify(cardId) +  ', "title": ' + JSON.stringify(title) +  ', "outgoing": ' + JSON.stringify(outgoing) + ' },\n';
};
javascript.javascriptGenerator.forBlock['Link'] = function(block) {
  ...
  var target = parseBlocklyListField(block.getFieldValue('target'));
