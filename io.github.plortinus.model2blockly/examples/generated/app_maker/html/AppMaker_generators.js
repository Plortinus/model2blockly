...
javascript.javascriptGenerator.forBlock['DataSource'] = function(block) {
  ...
  var retryCount = block.getFieldValue('retryCount');
  var headers = parseBlocklyListField(block.getFieldValue('headers'));
  return '{' +   '"_type": "DataSource", "_blockId": ' + JSON.stringify(block.id) +  ', "_modelId": ' + JSON.stringify(name || block.id) +  ', "name": ' + JSON.stringify(name) +  ', "url": ' + JSON.stringify(url) +  ', "method": ' + JSON.stringify(method) +  ', "cacheEnabled": ' + JSON.stringify(cacheEnabled) +  ', "retryCount": ' + JSON.stringify(retryCount) +  ', "headers": ' + JSON.stringify(headers) + ' },\n';
};
...
javascript.javascriptGenerator.forBlock['Page'] = function(block) {
  ...
  var tags = parseBlocklyListField(block.getFieldValue('tags'));
  ...
  return '{' +   '"_type": "Page", "_blockId": ' + JSON.stringify(block.id) +  ', "_modelId": ' + JSON.stringify(route || block.id) +  ', "title": ' + JSON.stringify(title) +  ', "route": ' + JSON.stringify(route) +  ', "layout": ' + JSON.stringify(layout) +  ', "navigationStyle": ' + JSON.stringify(navigationStyle) +  ', "backgroundColor": ' + JSON.stringify(backgroundColor) +  ', "scrollable": ' + JSON.stringify(scrollable) +  ', "tags": ' + JSON.stringify(tags) +   ', "components": ' + (components_stmt ? '[' + components_stmt + ']' : '[]') +   ', "onEnter": ' + (onEnter_stmt ? '[' + onEnter_stmt + ']' : '[]') + ' },\n';
};
...
javascript.javascriptGenerator.forBlock['SelectInput'] = function(block) {
  var name = block.getFieldValue('name');
  var labelText = block.getFieldValue('labelText');
  var optionsText = parseBlocklyListField(block.getFieldValue('optionsText'));
...
javascript.javascriptGenerator.forBlock['FileUpload'] = function(block) {
  var name = block.getFieldValue('name');
  var labelText = block.getFieldValue('labelText');
  var acceptTypes = parseBlocklyListField(block.getFieldValue('acceptTypes'));
...
javascript.javascriptGenerator.forBlock['TableView'] = function(block) {
  var columnsText = parseBlocklyListField(block.getFieldValue('columnsText'));
  var emptyText = block.getFieldValue('emptyText');
  var source = block.getFieldValue('source');
...
javascript.javascriptGenerator.forBlock['Form'] = function(block) {
  ...
  var requiredFields = parseBlocklyListField(block.getFieldValue('requiredFields'));
  var children_code = javascript.javascriptGenerator.statementToCode(block, 'children');
  var children_stmt = (children_code || '').trim().replace(/,\\s*$/, '');
  ...
  return '{' +   '"_type": "Form", "_blockId": ' + JSON.stringify(block.id) +  ', "title": ' + JSON.stringify(title) +  ', "endpoint": ' + JSON.stringify(endpoint) +  ', "requiredFields": ' + JSON.stringify(requiredFields) +   ', "children": ' + (children_stmt ? '[' + children_stmt + ']' : '[]') +   ', "onSubmit": ' + (onSubmit_stmt ? '[' + onSubmit_stmt + ']' : '[]') + ' },\n';
};
...
'DataSource': {
  ...
  fields: ['name', 'url', 'method', 'cacheEnabled', 'retryCount', 'headers'],
  fieldTypes: {'name': 'TEXT', 'url': 'TEXT', 'method': 'DROPDOWN', 'cacheEnabled': 'BOOLEAN', 'retryCount': 'INTEGER', 'headers': 'TEXT'},
  references: [],
  values: [],
  statements: []
},
...
'Page': {
  ...
  fields: ['title', 'route', 'layout', 'navigationStyle', 'backgroundColor', 'scrollable', 'tags'],
  fieldTypes: {'title': 'TEXT', 'route': 'TEXT', 'layout': 'DROPDOWN', 'navigationStyle': 'DROPDOWN', 'backgroundColor': 'COLOUR', 'scrollable': 'BOOLEAN', 'tags': 'TEXT'},
  references: [],
  values: [],
...
'Form': {
  ...
  fields: ['title'],
  fieldTypes: {'title': 'TEXT'},
  references: ['endpoint', 'requiredFields'],
  values: [],
