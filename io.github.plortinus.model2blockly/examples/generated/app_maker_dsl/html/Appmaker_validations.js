...
function computeValidationWarnings(workspace) {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Button') {
			...
			if (!prev || prev.type !== 'TextLabel')
				queueWarn(block, '"Button" must be preceded by "TextLabel".');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'App') {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Container') {
			...
			if (count > 40)
				queueWarn(block, '"Container.children" allows at most 40 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Form') {
			...
			while (child) { count++; child = child.getNextBlock(); }
			if (count < 1)
				queueWarn(block, '"Form.children" needs at least 1 element(s). Has: ' + count);
			if (count > 30)
				queueWarn(block, '"Form.children" allows at most 30 element(s). Has: ' + count);
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Form') {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'ImageView') {
			var val = block.getFieldValue('altText');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"ImageView.altText" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'ListView') {
			var val = block.getFieldValue('emptyText');
			if (val === null || val === undefined || val === '')
				queueWarn(block, '"ListView.emptyText" is required (cannot be empty).');
		}
	});
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'ListView') {
	...
	workspace.getAllBlocks(false).forEach(function(block) {
		if (block.type === 'Page') {
			...
			if (!result.ok) {
				var message = "Page routes must start with /";
				if (result.error) message += ' Expression error: ' + result.error;
				queueWarn(block, message);
			}
		}
